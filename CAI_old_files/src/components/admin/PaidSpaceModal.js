...
function PaidSpaceModal({ closePaidSpaceModal, getSpaces, type, tempSpace }) {
  ...
  const validateFields = () => {
    const requiredFields = [
      { key: 'paid_space_name', label: '空間名稱' },
      { key: 'paid_space_location', label: '空間位置' },
      { key: 'paid_capacity', label: '容納人數' },
      { key: 'paid_facilities', label: '設施描述' },
      { key: 'paid_space_overview', label: '空間概述' },
      { key: 'paid_floor_space_url_1', label: '照片連結 1' },
    ];
  ...
  const submit = async () => {
    ...
    try {
      ...
      handleSuccessMessage(dispatch, '操作成功', type === 'edit' ? `編輯 ${tempData.paid_space_name} 付費空間成功` : '創建 新的一筆付費空間 成功');
      closePaidSpaceModal();
      getSpaces();
    } catch (error) {
