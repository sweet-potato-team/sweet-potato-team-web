...
function NavBar({ sectionRefs, isAdmin }) {
  const navigate = useNavigate(); // 使用 navigate 來返回上一頁
  ...
  return (
    <NavBarContainer className="NavBar">
      <Spacer />
        <LeftSection>
          {/* BackButton 點擊後返回上一頁 */}
          <BackButton className="bi bi-reply-fill" onClick={() => navigate(-1)}></BackButton>
          <Title>樹洞系統</Title>
        </LeftSection>
      <Spacer />
      <RightSection>
        ...
        {isAdmin && (
          <NavBarItem onClick={() => handleClick(sectionRefs.dataList, '資料列表')}>
            <i className="bi bi-folder"></i>
            <span className="tooltip-text">資料列表</span>
          </NavBarItem>
        )}
      </RightSection>
    </NavBarContainer>
