...
function Step1({ nextStep, setRentalData, spaceName }) {
  ...
  const handleConfirm = () => {
    let startDateTime1 = null;
    let endDateTime1 = null;
    let startDateTime2 = null;
    let endDateTime2 = null;
    ...
    selectedSlots.forEach((selectedSlot, index) => {
      ...
      if (index === 0) {
        ...
        startDateTime1 = new Date(slotDate);
      }
  
      if (index === selectedSlots.length - 1) {
        slotDate.setUTCHours(endHour, endMinute, 0);
        endDateTime1 = new Date(slotDate);
      }
  
      if (index === 1) {
        slotDate.setUTCHours(startHour, startMinute, 0);
        startDateTime2 = new Date(slotDate);
      }
  
      if (index === selectedSlots.length - 2) {
        slotDate.setUTCHours(endHour, endMinute, 0);
        endDateTime2 = new Date(slotDate);
      }
    });
  
    const formattedStartDateTime1 = startDateTime1.toISOString().slice(0, 16);
    const formattedEndDateTime1 = endDateTime1.toISOString().slice(0, 16);
    const formattedStartDateTime2 = startDateTime2 ? startDateTime2.toISOString().slice(0, 16) : null;
    const formattedEndDateTime2 = endDateTime2 ? endDateTime2.toISOString().slice(0, 16) : null;
